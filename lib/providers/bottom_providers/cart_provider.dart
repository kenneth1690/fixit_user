...
class CartProvider with ChangeNotifier {
          ...
          if (element.value.isPackage! == false) {
            ...
            for (var ser in element.value.servicePackageList!.services!) {
              ...
              if (ser.selectServiceManType != "app_choose") {
                ...
                idList = [];
                if (ser.selectedRequiredServiceMan == null) {
  ...
  editCart(CartModel cart, context, index) async {
    if (cart.isPackage!) {
      ...
      ProviderModel provider =
          await commonApi.getProviderById(cart.serviceList!.userId);
      ...
      onBook(context, cart.serviceList!,
          provider: cart.serviceList!.user ?? provider,
          addTap: () => onAdd(index),
  ...
  onRemoveService(context, index) async {
    if (int.parse(cartList[index].serviceList!.selectedRequiredServiceMan!) ==
      ...
      if (int.parse(cartList[index].serviceList!.requiredServicemen!) ==
        ...
        isAlert = false;
        notifyListeners();
        cartList[index].serviceList!.selectedRequiredServiceMan = (int.parse(
